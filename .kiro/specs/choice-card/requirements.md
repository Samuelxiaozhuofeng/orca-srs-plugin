...
- **Choice Option（选择项）**: 选择题卡片的直接子块，代表一个可选答案
- **Correct Option（正确选项）**: 带有 `#correct` 或 `#正确` 标签的选择项
- **Single-Choice（单选题）**: 只有一个正确选项的选择题
- **Multiple-Choice（多选题）**: 有多个正确选项的选择题
...
2. WHEN displaying options THEN the SRS System SHALL only render direct child blocks and exclude nested children (grandchildren)
3. WHEN rendering options THEN the SRS System SHALL use block rendering to display each option with full content formatting
...
5. WHEN a single-choice card is displayed THEN the SRS System SHALL use radio button style interaction (click to select one)
6. WHEN a multiple-choice card is displayed THEN the SRS System SHALL use checkbox style interaction with a submit button
7. WHEN the user clicks on an option in single-choice mode THEN the SRS System SHALL select that option and deselect others
8. WHEN the user clicks on an option in multiple-choice mode THEN the SRS System SHALL toggle that option selection state
9. WHEN the user changes selection before revealing the answer THEN the SRS System SHALL allow modifying the selection
10. WHEN the answer is revealed THEN the SRS System SHALL record the final selection and highlight correct options with green styling
11. WHEN the answer is revealed THEN the SRS System SHALL indicate incorrect selections with red styling

### Requirement 4

**User Story:** As a user, I want options to be shuffled during review, so that I learn the content rather than memorizing positions.
...
2. WHEN in multiple-choice mode THEN the SRS System SHALL support Enter key to submit the selection
3. WHEN the answer is revealed THEN the SRS System SHALL support the same grade shortcuts (1-4 for Again/Hard/Good/Easy) as other card types
...
2. WHEN the user selects any incorrect option or misses any correct option THEN the SRS System SHALL pre-select "Again" grade
3. WHEN the grade is pre-selected THEN the SRS System SHALL allow the user to manually change to a different grade
