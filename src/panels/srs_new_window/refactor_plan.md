...
- [x] 创建独立的 `DirectionCardRenderer.tsx` 组件
- [x] 移除 direction 卡片的过滤条件
- [x] 支持 direction 卡片评分（`updateDirectionSrsState`）
- [x] 渲染路由支持 direction 卡片

**验证**：

- [x] pnpm build 编译通过
- [ ] 正向复习时问题在左、答案在右（需手动测试）
- [ ] 反向复习时问题在右、答案在左（需手动测试）
- [ ] 评分后正确更新对应方向的 SRS 状态（需手动测试）
...
- [ ] 删除旧的 `SrsReviewSessionRenderer.tsx`
