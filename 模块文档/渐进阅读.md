...
说明：默认值由插件设置项 `enableAutoExtractMark` 控制。
...
- `ir.postponeCount`：累计推后次数（手动推后 + 超额自动后移）
...
优先级调度分为两层，并引入“间隔增长”以避免材料长期高频出现：

1) `#card.priority`（高/中/低）用于 Extract 的排期，区间随机化，避免同日堆积：

- 高优先级 → 1-2 天
- 中优先级 → 3-5 天
- 低优先级 → 7-10 天

2) `ir.priority`（1-10）用于 Topic 的排期，固定映射如下：
...
// 新增：基于 intervalDays 的到期计算与增长
// - 已读：intervalDays *= growthFactor，再算 due
// - 改优先级：intervalDays 重置为“基础间隔”，并立刻重算 due
```
...
- 靠前/靠后：等同“已读 + 优先级上/下调一档”
  - Topic：调整 `ir.priority`（低/中/高对应 2/5/9）
  - Extract：仅调整 `#card.priority`（高/中/低）
- 推后：把当前卡片推迟到未来几天（按优先级档位自动选择天数），并写回 `ir.due/ir.intervalDays/ir.postponeCount/ir.lastAction`
