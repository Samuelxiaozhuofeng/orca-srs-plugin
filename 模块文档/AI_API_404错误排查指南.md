...
> 文档类型：排查类（不扩写为完整模块文档）
> 文档同步日期：2026-07-16
> 变更说明：与 Plan B `aiConfigValidator.ts` / `aiService.ts` 对齐；仅 OpenAI 兼容 chat/completions。
...
闪卡生成（`generateFlashcardDrafts`）与「测试连接」（`testAIConfigWithDetails`，含有限超时）均 `POST` 到设置中的 **`ai.apiUrl` 完整字符串**；URL 少路径时最常见表现是 **HTTP 404**。

**不要**配置 Ollama 原生 `…/api/chat`：校验会拒绝；请使用 `…/v1/chat/completions`。
...
1. 本地 `validateAIConfig`（Key、URL、协议；拒绝 `/api/chat`；常见服务路径警告）
2. 向配置 URL 发最小 `POST`（`max_tokens: 5`，有限超时）
3. 失败时保留截断后的响应正文，并 `formatAIConfigError` 给出诊断
...
- 默认 HTTP + 端口 `11434`
- 校验器对 localhost：路径宜含 `/v1/chat/completions`（拒绝原生 `/api/chat`）
...
| 单次草稿生成 | `src/srs/ai/aiService.ts` → `generateFlashcardDrafts` |
| 校验、示例、详细测试、404 文案 | `src/srs/ai/aiConfigValidator.ts` |
| 详细错误格式化 | `src/srs/ai/aiConfigValidator.ts`（`formatAIConfigError`） |
