...
章节选择会过滤 EPUB 的纯封面包装页。标题优先使用正文标题与 EPUB 目录；前置页不在目录中时，会尝试 HTML 标题和首段短文本，不再显示无意义的 `Chapter N`。
...
| 页面标题去重 | 仅当正文首个标题是 `h1` 且与章节页标题等价时，由 `getChapterContent` 去掉该 `h1`；`h2` 至 `h6` 和不匹配的 `h1` 必须保留 |
