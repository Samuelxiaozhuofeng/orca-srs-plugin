...
> 文档同步日期：2026-07-13  
> 变更说明：按 `src/importers/epub/*` 与 `src/components/epub-import/*` 校正文件树、流程、向导步骤与 BookIR 衔接边界。
...
| 文件 | 职责 |
| --- | --- |
| `types.ts` | `ParsedEpub`、`EpubBookManifestV1`、`ImportEpub*`、`EPUB_PROP`、`BookIRPlanV1` 契约、`EpubValidationError` |
| `fingerprint.ts` | `computeSha256Hex`（Web Crypto 或纯 JS 回退） |
| `epubParser.ts` | ZIP/OPF/spine/nav/NCX、`parseEpub`、章节 key、`getChapterContent` |
| `epubHtml.ts` | HTML 根节点、标题合并/去重、图片 src 改写、sanitize |
| `epubAssets.ts` | 章节内图片上传；`uploadSourceEpub` / `loadSourceEpubBuffer`（源文件断点续传） |
