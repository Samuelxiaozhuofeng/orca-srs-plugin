...
> 文档同步日期：2026-08-04
> 近期变更：
> 1. **多块 / 跨样式 / 子树源文**：同块跨 fragment、同父连续兄弟跨块；整段范围展开有界子树（深度 8 / 80 块、缩进、排除 #card 与 AI 结果预览）；结果锚点为阅读方向末块；`QUICK_SELECTION_MAX=12000`。
> 2. （历史）传输层统一、制卡弹窗 v2、选择题卡、队列 `batchId` + `pending` 等见既有实现。
>
> **能力边界**：跨块只按 anchor↔focus 的连续兄弟闭区间解析，不读取「跳选」块 ID 集合；行内端点切片不含端点子树；跨父选区真机表现仍以宿主为准。
>
...
| 路径 | 入口 | 行为 |
| --- | --- | --- |
| **AI 生成闪卡** | `${pluginName}.makeAICard`（别名 `interactiveAICard`） | 跨块选区用拼接文本，否则读当前光标块全文 → 弹窗配置 → AI 请求 → 校验/预览/编辑/勾选 → 确认后分组写入（锚点为末块或当前块） |
| **块解释** | 渐进阅读会话：移到块右侧隐形热区出「?」或 `Alt+E` | 读目标块 → 白话/名词内联；可选举例/反驳/追问；用户点「+」才写入普通子块 |
| **AI 快捷交互** | 编辑器工具栏 sparkles 按钮 | 选中文本（同 fragment / 同块跨样式 / 同父相邻跨块）→ 选提示词；结果挂在**选区末块**下；见下「快捷交互」 |
| **AI 快捷制卡** | `quickBasicCard` / `quickClozeCard` / `quickChoiceCard`（斜杠「快捷问答卡 / 快捷填空卡 / 快捷选择题」） | 选中文本（或整块；支持跨块）→ 直接生成 → 以**待激活卡片**插到**末块**下方预览 → 保留 / 丢弃 |
...
| 同父相邻兄弟跨块（行内拖选） | 首/末块按 offset 切片（**不含**端点子树）；中间块全文 + **有界子树**；块间 `\n`；`blockId` = **阅读方向末块**；**关闭**块上下文 |
| 块级 / 整段范围 | `isInline===false` 或两端块首 `(0,0)`：起止闭区间各块 **全文 + 有界子树**（2 空格缩进表示层级） |
...
| 不支持 | 不同父、端点不在同一 `children` 链 → 可见 warn；跨块任意失败（含 empty）**不**退回锚点全文 |
...
源文本取法：优先选区（含同块跨样式 / 同父相邻跨块，块间换行拼接），无选区则用锚点块正文（「光标停在块里直接按快捷键」是最顺手的用法，不该报错）；跨块时结果挂在**末块**下；不同父/非兄弟链可见报错而非静默退回。
