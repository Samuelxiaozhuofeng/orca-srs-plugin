...
> 文档同步日期：2026-07-16
> 变更说明：Plan B 修复轮——本地唯一 draft id、加强接地、截断计数分离、生成 token 防竞态、尽力回滚与残留 ID 报告、单斜杠命令、配置诊断对齐。

## 概述

AI 模块提供基于 **OpenAI 兼容 Chat Completions** 的源文本制卡能力。产品路径只有一条：

| 路径 | 命令 | 行为 |
| --- | --- | --- |
| **AI 生成闪卡** | `${pluginName}.makeAICard`（编辑器兼容别名 `interactiveAICard`） | 读当前光标块 → 弹窗配置 → **一次** AI 请求 → 校验/预览/编辑/勾选 → 确认后分组写入 |

可见斜杠命令仅 **`${pluginName}.aiCard`（AI 生成闪卡）**。
...
`aiService.test.ts`、`aiDraftParseValidate.test.ts`、`aiCardWriter.test.ts`、`aiRequestToken.test.ts`、`aiConfigValidator.test.ts`
