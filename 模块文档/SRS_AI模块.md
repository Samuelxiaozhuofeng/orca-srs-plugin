...
- **后台路径**（`directWriteBelow` 或 `insertBelowOnComplete`；`startBackgroundQuickInsertJob({ commitMode, tags, reuseSameResultBlock })`）：
  ...
  5. **失败**：生成失败（非 `CANCELLED`）或插入/打标失败 → job `status=error` + `errorMessage`（脱敏）+ `orca.notify("error", …, { title: "AI 快捷交互" })`。toast 是用户可见主通道；仅 HTTP 401/403 额外渲染错误卡，保留原错误并提供「打开连接设置」；未预期异常路径同
