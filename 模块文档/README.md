...
> - [渐进阅读_章末小测.md](渐进阅读_章末小测.md)：Topic 完成后 / 更多菜单 → 本章全文单选小测；**Custom Panel 专注答题** + 块侧紧凑入口；揭晓后扁平意图条（加入复习 / 问 AI / 原文）；可选简答/填空入队；与 `#choice` 路径独立
...
24a. **[渐进阅读_章末小测.md](渐进阅读_章末小测.md)** ⭐ 2026-08-01 更新 — Topic 章末小测；Custom Panel + 共享生成取消 / live 同步 / 错题回看制卡；panel 制卡不用 invokeGroup + 短暂切左侧编辑焦点；**制卡落点=当前题 sourceBlockId 子块** + 只剩 Panel 时自动打开原文 block 视图；panel nav A→B
