...
24a. **[渐进阅读_章末小测.md](渐进阅读_章末小测.md)** ⭐ 2026-08-01 更新 — Topic 章末小测；Custom Panel + 共享生成取消 / live 同步 / 错题回看制卡；panel 制卡不用 invokeGroup + 短暂切左侧编辑焦点；**制卡落点=当前题 sourceBlockId 子块** + 只剩 Panel 时自动打开原文 block 视图；「原文」跳转优先左侧 srs.ir-session 阅读面板（正文内定位），无阅读面板才开/复用右侧侧栏；panel nav A→B
