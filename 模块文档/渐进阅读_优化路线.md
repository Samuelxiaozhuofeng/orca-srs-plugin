...
| P2 | IR 会话 Extract→Cloze/Q&A | **已落地** | Cloze + **Q&A + Direction** 共用 `irConversionService` 事务脚手架；会话更多菜单「问答」「方向」（`keep_extract`） |
...
- ~~Extract→**Q&A/Direction** 的 IR 原子转化路径~~ → **2026-07-26 已落地**（`convertExtractToQA` / `convertExtractToDirection`）
...
- 会话 UI 路径：正文仍写 `IncrementalReadingSessionDemo.tsx` 为主——**过时**；现行为 `IRWorkspaceShell` + `IRSessionShell`（该组件已于 2026-07-26 从仓库删除）
...
历史/兼容：`IncrementalReadingSessionDemo.tsx` **已于 2026-07-26 删除**（零引用死兼容层，低危#8）。

- 下一篇/已读：`markAsRead` / `performNext`（间隔增长 + stage）
- 重要性：三档映射 `ir.priority`（比例修正 interval）
- 归档/完成本章：`performArchive` / `completeIRCard`；顺序书走 Book IR progression，完成本章时显式选择下一章 `today`/`tomorrow` due
...
- [x] 把“跳过”改为“推后（Postpone）”，并写回 `ir.due`（历史落点 `IncrementalReadingSessionDemo.tsx`——该组件已于 2026-07-26 删除，现行推后在 `IRSessionShell`；`src/srs/incrementalReadingStorage.ts` 的 `postpone` API 仍在）
...
- [x] IR 会话 Extract→**Cloze** 原子转化（`irConversionService.convertExtractToItem`、`Alt+Z` / 动作栏「挖空」；失败补偿）
- [x] Extract→**Q&A/Direction** 原子路径（2026-07-26）：`convertExtractToQA` / `convertExtractToDirection` 复用同一事务/回滚脚手架；证据：`src/srs/incremental-reading/irConversionService.ts`、`irConversionService.test.ts`；UI：`useIRSessionCardActions` + `IRSessionMorePanel`「问答」「方向」
...
- **已落地补充（2026-07）**：顺序 Book IR 的 `activeChapterId` 章使用 **SAC** 短节奏（`1+2*(1-p/100)`，无 1.25 增长，停滞上限约 6 天），与上列记忆型增长分离。见 `模块文档/渐进阅读_BookIR.md`。
...
> 历史“开工导航”。**2026-07-13**：多数条目已落地；会话主路径已迁至统一工作区。**2026-07-26**：`IncrementalReadingSessionDemo` 已从仓库删除，正文提及处均为历史记载。
...
- 会话 UI（现行）：`IRWorkspaceShell` / `IRSessionShell` / `workspace/*`；Demo 组件仅遗留
  - 推后、隐藏内部字段、Extract→Cloze / Q&A / Direction — **已落地**
