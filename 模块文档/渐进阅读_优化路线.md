...
- 下一次到期：由 `ir.priority(0-100)` + cardType 计算基础间隔，并加入少量抖动（`src/srs/incrementalReadingScheduler.ts`）
- 队列构建：`buildIRQueue`（`src/srs/incrementalReadingCollector.ts`）
  ...
  - 超额自动后移：`deferOverflowCards`（按优先级推后，并写回 `due/intervalDays/postponeCount/lastAction`）
