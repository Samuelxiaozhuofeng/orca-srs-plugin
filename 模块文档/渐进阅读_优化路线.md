...
> 目标：把当前插件的 Topic/Extract 机制升级为更贴近 SuperMemo 的“漏斗式提纯”工作流：**Topic → Extract → Item（Cloze/Q&A）**，并让调度真正服务于“在有限时间内优先处理高价值信息”，而不是反复机械刷同一篇材料。
...
- `ir.readCount`: number
- `ir.due`: Date
- `ir.position`: number | null（Topic 队列位置）
...
- Topic 下一次到期：`ir.priority(1-10)` → 固定间隔（`src/srs/incrementalReadingScheduler.ts`）
- Extract 下一次到期：优先使用 `#card.priority(高/中/低)` 的随机区间（同上；由 `incrementalReadingStorage.ts#computeNextDue` 在 `cardType==="extracts"` 时生效）
- 队列构建：`buildIRQueue`（`src/srs/incrementalReadingCollector.ts`）
  - Topic：按 `ir.position` 升序
  - Extract：按 `due` 升序，再按 `state.priority`（注意：不是 `#card.priority`）
  - 交错混排：`topicQuotaPercent` + `dailyLimit`
  - 超额自动后移：`deferOverflowCards`
    - Extract：直接把 `ir.due` 推到“明天 00:00”
    - Topic：只把 `ir.position` 移到队尾
...
- 靠前/靠后：
  - Topic：调整 `ir.priority`（并已读）
  - Extract：调整 `#card.priority`（并已读）
...
现状：
- Extract 的下一次 due 用的是 `#card.priority`（高/中/低）随机区间。
- 但队列排序用的是 `IRCard.priority = ir.priority`（常见为默认 5），不等同于 `#card.priority`。

后果：同日多个 Extract 的先后顺序可能不符合你对“高/中/低”的预期。
...
### 2.4 自动后移（Auto-postpone）不区分优先级，且 Topic 只改 position

现状：超额时全部溢出卡都被后移；Extract 只推到明天；Topic 只调 position。  
建议方向：Auto-postpone 应该“优先推迟低优先级”，并把推迟体现为更合理的 `due`（不仅仅明天）。
...
- [x] 修正 Extract 的“有效优先级”用于排序（`src/srs/incrementalReadingCollector.ts`）
- [x] 自动后移仅推迟低优先级优先（`src/srs/incrementalReadingCollector.ts#deferOverflowCards` + `buildIRQueue` 选取逻辑）
...
- [x] 自动后移不再固定“明天 00:00”，而是按优先级推迟到未来几天
...
- 队列与自动后移：`src/srs/incrementalReadingCollector.ts`
  - Extract 排序改用 `#card.priority` 的有效优先级（P0）
  - `deferOverflowCards` 改为优先推迟低优先级，并写 `due`（P0/P1）
