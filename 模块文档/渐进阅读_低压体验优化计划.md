...
> **文档同步日期：2026-07-26** — 补充状态对照：§8.3「自动推后」设计已落地（会话启动触发，非仅按钮/测试）；新增「迟到补偿」「排序老化」「探索改真随机」三项落地能力（原计划未逐字列出，代码为准）。  
> 2026-07-13：仅补充状态对照；正文规划叙述保留作演进史料。  
...
| 队列排序改加性得分老化（终结低优先级饥饿）+ 探索改真随机采样 | `irQueuePolicyCore.ts`（`cardValue`/`sortByValue`/`sortVictimsLowValueFirst`）；2026-07-26 |
| 自动推后批次 + 撤销（进程内批次表） | `irOverloadService.ts`；字段 `ir.autoPostponeBatchId`。**2026-07-26 起接回主路径**：由用户显式启动会话触发（`runSessionStartAutoPostpone.ts` + `irAutoPostponeGuard.ts` 当日一次），此前仅供资料库/测试手动调用，见 §8.3 |
| 已读间隔增长补「迟到补偿」（拖延后才读不按存储间隔直接增长，防 backlog 自我强化） | `irSchedulingHelpers.ts`（`computeLatenessEffectiveBase`）；2026-07-26；不在原计划逐字列出，代码为准 |
...
| Extract→Q&A / Direction 原子事务 | **已落地**（2026-07-26）：与 Cloze 共用 `irConversionService` 脚手架；会话更多「问答」「方向」 |
...
> **落地对照（2026-07-26）**：上述五步已实现为 `runSessionStartAutoPostpone.ts`，由用户显式启动会话（10/20/30 `autoStart` / `beginReading` / `onStartSession`）触发，先于队列装配完成并 `await`；当日一次守卫用 localStorage（`irAutoPostponeGuard.ts`）而非内存标记；「按重要性分散到未来日期」用 `getPostponeDays`；额外新增「反复推迟联动降权」（`postponeCount>=3` 时 `priority -= 2`，下限 5，对应 SuperMemo 语义，原计划未列出）。详见 [`渐进阅读.md`](渐进阅读.md)「队列策略与过载」。
...
src/components/incremental-reading/
  ...
  IRActionBar.tsx             # 下一篇、摘录|挖空、重要性、完成、⋯(更多)
...
- **与原计划的偏差**：
  ...
  - Extract→Q&A / Direction 已于 2026-07-26 落地：`convertExtractToQA` / `convertExtractToDirection` + 更多菜单入口（与 Cloze 共用事务回滚）
