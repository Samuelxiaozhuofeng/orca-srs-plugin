...
- Demo **不**导入/调用可变全局 deck filter；启动后即使全局 filter 被改成其他牌组，当前会话 scope 不变。
- 候选建队使用**不限额** `buildReviewQueue(..., null)`（仍做 FC-11 稳定排序），再经 scope + budget 截取。
...
| 最小延迟 | `PENDING_TIMER_MIN_DELAY_MS = 1000`；到期后缓冲 `PENDING_TIMER_POST_DUE_BUFFER_MS = 500` |
...
| 完成摘要 reopen | 完成态下 pending **实际追加 ≥1** 时：`reopenSessionFinalizeIfNeeded` 重置 finalize 缓存并 `setSessionStats(null)`，**不清零** progress；`resumeSessionPersistence` / `resumeSessionProgressAutosave` 恢复 autosave。未实际追加不得重置摘要。第二次完成摘要须含此前评分 + 重入评分 |

### 异步加载 (`src/srs/asyncLoadGeneration.ts`)

Renderer 快速切换/重试时：`createLoadGenerationGate` + `decideLoadCommit`；仅最新 generation 可提交 state，旧 in-flight 结果丢弃（stale）。
...
主页统计刷新与会话 scope 无关；启动某牌组复习时由 `startReviewSession(deckName)` 创建带 deck scope 的 descriptor 会话块，由 Renderer 读块冻结。
...
4. **资源清理**: 组件卸载时清理定时器与 pending
5. **收集优化**: FC-13 预热/分批降低 60s 扫描成本（见 `SRS_复习队列管理.md`）
...
| `src/srs/reviewSessionScope.test.ts` | FC-02 / FC-14 回归 |
| `src/srs/reviewSessionBudget.ts` | 每日额度与会话 budget |
| `src/srs/reviewSessionBudget.test.ts` | 额度回归 |
| `src/srs/pendingDueRequeue.ts` | F2-04 Again/Hard 短期 pending |
| `src/srs/pendingDueRequeue.test.ts` | 尾部去重 / token / fake timer |
| `src/srs/asyncLoadGeneration.ts` | 异步 latest-wins |
| `src/srs/asyncLoadGeneration.test.ts` | generation gate 回归 |
| `src/srs/sessionProgressStorage.ts` | 完成后再开时 resume autosave |
| `src/srs/cardCollector.ts` | `collectReviewCards` / `buildReviewQueue` |
| `src/components/SrsReviewSessionRenderer.tsx` | 启动时创建并传递 scope/budget |
