...
> - 补齐 `extractCardType` 全类型、cardCollector 多卡种、panelUtils `findLeftPanel`、sessionProgress 等以代码为准的导出  
> - 相关文件改为仓库内相对路径
...
公开 API 仍从 `src/srs/statisticsManager.ts` 导入；实现按职责拆分：

| 文件 | 职责 |
| ---- | ---- |
| `src/srs/statistics/statisticsCache.ts` | 缓存键、TTL、失效 |
| `src/srs/statistics/reviewActivityCalculations.ts` | 日志/活动类纯计算（今日、历史、时间、答题按钮） |
| `src/srs/statistics/cardDistributionCalculations.ts` | 卡片/分布类纯计算（预测、状态、间隔、难度） |
| `src/srs/statistics/statisticsLoaders.ts` | 缓存包装 + 日志/卡片加载 |
| `src/srs/statistics/statisticsPreferences.ts` | 时间范围/牌组偏好持久化 |

今日 `totalTime`、趋势 `dailyTime` 等对 `log.duration` 的累计均应经 `effectiveDurationFromReviewLog`，与会话摘要同口径。

相关：`sessionProgressStorage.ts`、`sessionProgressFinalize.ts`（会话进度持久化与收尾，细节见复习窗口/队列文档）。
...
| `src/srs/statistics/*` | 统计缓存 / 活动计算 / 分布计算 / 加载 / 偏好 |
| `src/components/SrsFlashcardHome.tsx` | Home UI（非旧 SrsCardBrowser） |
