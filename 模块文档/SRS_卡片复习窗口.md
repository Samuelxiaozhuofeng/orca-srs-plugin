...
| 设置键 | 说明 | UI 入口 |
| ------ | ---- | ------ |
| `review.disableNotifications` | 关闭通知提醒 | 服务设置面板 **复习** 页（helper：`reviewAdvancedSettings.ts`）；同时注册在原生 schema（否则无法持久化） |
| `review.newCardsPerDay` | 每日新卡上限（0..10000；0=当天不安排新卡） | 服务设置面板 **复习** 页（helper：`reviewServiceSettings.ts`）；同时注册在原生 schema（否则无法持久化） |
...
| `review.fsrsWeights` | FSRS 权重（21 个） | 服务设置面板 **复习** 页「算法参数」小节（helper：`reviewAdvancedSettings.ts`）；同时注册在原生 schema；算法 runtime +「恢复 FSRS 默认」命令 |
| `review.fsrsMaximumInterval` | 最大间隔（天） | 同上 |

> 每日额度与 FSRS 参数仍存 plugin settings 原 key，算法/额度读取路径不变；复习页含额度/保留率 + 两项界面开关（Pass-Fail、显示下次时间），以及「算法参数」小节的权重/最大间隔、图片遮罩模式与关闭通知。原生插件设置面板保留全部条目作为兜底入口（未注册进 schema 的 key 无法持久化）。详见 [SRS_记忆算法.md](SRS_记忆算法.md)。
>
