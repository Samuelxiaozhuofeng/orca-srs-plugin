...
> **版本确认 ≠ 发布 Go。** 当前元数据版本为 `1.0.11`，但 `release-evidence/release-readiness.json` 在 Orca 真机与证据门禁完成前为 No-Go；正式 `orca-srs-1.0.11.zip` 必须绑定已验收 `dist/index.js` 的 SHA-256、干净提交和指向该提交的 `v1.0.11` tag。推送 tag 后由 tag-only workflow 复验并发布。
