...
> **版本确认 ≠ 发布 Go。** 当前元数据版本为 `1.0.4`，但 `release-evidence/release-readiness.json` 在 Orca 真机与证据门禁完成前为 No-Go；正式 `orca-srs-1.0.4.zip` 必须绑定已验收 `dist/index.js` 的 SHA-256、干净提交和指向该提交的 `v1.0.4` tag。推送 tag 后由 tag-only workflow 复验并发布。
...
1. 在同一块内选中文本（可跨加粗/链接等样式片段），点击编辑器工具栏的 ✨。
2. 菜单会标明该提示词是「点击即生成」还是「弹窗后生成」、发送范围与模型。
3. 后台任务可随时取消；失败会保留错误并支持重试。
4. 生成结果先作为当前块的临时子块预览，可保留、追问、再生成、复制、移为同级或删除。
5. 临时预览离开当前页面会删除；本次插件会话内可从 Quick AI 的「最近删除」恢复。
...
复习等常规设置位于：**设置 → 插件 → 虎鲸标记**。AI 连接使用顶部栏插头图标或斜杠命令 **「AI / Firecrawl 服务设置」**。
